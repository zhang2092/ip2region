...
func (region *Ip2Region) BinarySearch(ipStr string) (ipInfo IpInfo, err error) {
	ipInfo = IpInfo{}
	if region.totalBlocks == 0 {
		_, err := region.dbFileHandler.Seek(0, 0)
		if err != nil {
			return ipInfo, err
		}

		superBlock := make([]byte, 8)
		_, err = region.dbFileHandler.Read(superBlock)
		if err != nil {
			return ipInfo, err
		}
	...
	_, err = region.dbFileHandler.Seek(dataPtr, 0)
	if err != nil {
		return ipInfo, err
	}

	data := make([]byte, dataLen)
	_, err = region.dbFileHandler.Read(data)
	if err != nil {
		return ipInfo, err
	}
...
func (region *Ip2Region) BtreeSearch(ipStr string) (ipInfo IpInfo, err error) {
	...
	if region.headerLen == 0 {
		_, err = region.dbFileHandler.Seek(8, 0)
		if err != nil {
			return ipInfo, err
		}
	...
	blockLen := eptr - sptr
	_, err = region.dbFileHandler.Seek(sptr, 0)
	if err != nil {
		return ipInfo, err
	}

	index := make([]byte, blockLen+IndexBlockLength)
	_, err = region.dbFileHandler.Read(index)
	if err != nil {
		return ipInfo, err
	}
